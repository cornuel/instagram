...
const { medias, currentMedia } = storeToRefs(
  useCreatePostStore()
)
...
<template>
  <div
    class="list-post-container h-[118px] w-full max-w-[374px] p-2 mx-4
      bg-overlay rounded-lg flex items-center justify-end
      flex-nowrap z-[1]"
  >
    <div
    ...
    >
      ...
      >
        <template #item="{ element: media }">
          <div
            class="slider-item relative px-[6px] cursor-pointer
              has-[ghost]:opacity-0"
            :class="{
              active: currentMedia!.url == media.url
            }"
          ...
          >
            ...
            ></div>
            <div
              class="absolute top-[5px] right-[10px] w-5 h-5 bg-overlay
                rounded-full hidden parent-[.slider-item.active]:flex
                flex-center cursor-pointer transition-all duration-200
                hover:opacity-60"
      ...
      <div class="navigation">
        <div
          class="navigation-prev left-0 absolute
            has-[slider-button-disable]:hidden top-1/2 -translate-y-1/2
            cursor-pointer z-[1]"
        >
          <fa
        ...
        <div
          class="navigation-next right-0 absolute
            has-[slider-button-disable]:hidden top-1/2 -translate-y-1/2
            cursor-pointer z-[1]"
        >
          <fa
    ...
    <div
      class="relative w-12 h-12 flex flex-center flex-shrink-0 border
        border-separator rounded-full cursor-pointer"
    >
      <fa
        size="xl"
        style="color: #fff"
