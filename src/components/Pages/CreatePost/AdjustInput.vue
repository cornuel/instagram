...
<template>
  <div class="group/adjust-item py-2">
    <div class="flex py-[14px] justify-between capitalize">
      ...
      <span
        v-if="inputValue != 0"
        class="font-semibold text-buttonColor-primary cursor-pointer hidden
          group-hover/adjust-item:block"
        @click="handleReset"
        >Reset</span
      >
    </div>
    <div class="flex flex-center cursor-pointer">
      <input
        ...
        v-model="inputValue"
      />
      <span class="text-xs w-6 ml-2 text-right">{{
        inputValue
      }}</span>
    </div>
  </div>
